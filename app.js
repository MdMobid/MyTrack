...
(() => {
  ...
  function showToast(message, type = 'info', duration = 3000) {
    ...
    toast.innerHTML = `<span class="toast__icon">${icons[type]}</span><span class="toast__message">${message}</span>`;
  ...
  // ── Persistence & Two-Way Merge Sync ──────────────────────
  function saveState() {
    ...
    // Non-blocking sync to Firebase Realtime DB
    if (window.db && window.db.isConfigured()) {
      ...
      }).catch(e => console.warn('Sync pending (offline or network error):', e));
    }
  }
  ...
  function loadState() {
    ...
    // Register two-way merge sync handler with DataAPI
    if (window.db) {
      window.db.registerSyncHandler('habits_state', async (remoteInfo) => {
        if (!remoteInfo) return;
        const merged = window.mergeHabitsState(
          { habits: state.habits, completions: state.completions, deletedHabitIds: state.deletedHabitIds },
          remoteInfo
        );
        ...
        // Propagate merged data back to remote so other devices receive the unified state
  ...
  function renderDailyView(container) {
    ...
    const activeHabits = state.habits.filter(h => !h.isPaused && h.days.includes(todayIdx));
    const inactiveHabits = state.habits.filter(h => !h.isPaused && !h.days.includes(todayIdx));
    const pausedHabits = state.habits.filter(h => !!h.isPaused);

    const todayComps = state.completions[today] || {};
    const completed = activeHabits.filter(h => todayComps[h.id]).length;
    const total = activeHabits.length;

    const incompleteFirst = [...activeHabits].sort((a, b) => {
      return (todayComps[a.id] || false) - (todayComps[b.id] || false);
    });

    const weekDates = getWeekDates();

    function renderHabitCard(habit, isActiveToday, isPaused = false) {
      ...
      if (!isActiveToday && !isPaused) cardClass += ' habit-card--inactive';
      if (isPaused) cardClass += ' habit-card--paused';
              ...
              <div class="habit-card__info">
                ...
                <div>
                  ...
                  <div class="habit-card__streak ${streak >= 3 ? 'on-fire' : ''}">
                    ${isPaused
          ? `<span class="habit-card__rest-tag" style="color:#fbbf24;">⏸️ ${streak > 0 ? `${streak} day streak (frozen)` : 'Streak frozen'}</span>`
          : !isActiveToday
            ...
            <div class="habit-card__bottom">
              ...
              const isActive = habit.days.includes(dayIdx);
              ...
              <div class="habit-card__actions">
                ${isPaused ? `
                  <button class="habit-card__action-btn resume-btn" onclick="window.__togglePauseHabit('${habit.id}')" title="Resume Habit">▶️</button>
                ` : `
                  <button class="habit-card__action-btn pause-btn" onclick="window.__togglePauseHabit('${habit.id}')" title="Pause Habit">⏸️</button>
                `}
          ...
          ` : `
            <div class="habits-grid">
              ...
              ${inactiveHabits.length > 0 ? `
                ...
                ${inactiveHabits.map(h => renderHabitCard(h, false, false)).join('')}
              ` : ''}
              ${pausedHabits.length > 0 ? `
                <div class="habits-section-divider" style="color:#fbbf24;">
                  <span>⏸️ Paused Habits (${pausedHabits.length})</span>
                </div>
                ${pausedHabits.map(h => renderHabitCard(h, false, true)).join('')}
              ` : ''}
            </div>
          `}
        </div>
  ...
  window.__toggleHabit = function (habitId, checked) {
    ...
    const activeToday = getActiveHabitsForDate(today);
    const todayComps = state.completions[today];
    const allDone = activeToday.length > 0 && activeToday.every(h => todayComps[h.id]);
  ...
  window.__deleteHabit = function (habitId) {
    ...
    if (!state.deletedHabitIds) state.deletedHabitIds = [];
    state.deletedHabitIds.push({ id: habitId, deletedAt: Date.now() });
